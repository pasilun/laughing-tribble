...
## Regression Test
`e2e/building-model.spec.ts`

## Description

Extend the design chat (spec design-screen.md) with LLM tool calls that extract a structured BuildingModel from the conversation. The model state is displayed as a summary panel so the user can see what the system has understood. No floor plan rendering yet.
...
- SVG floor plan (separate spec)
- Triage panel (separate spec)
- Validation/limit warnings (separate spec)

## Dependencies

- [[design-screen]] (chat UI and `POST /api/design/chat` route exist)
